...
$top10 = mssql_query("select  top 22 NUMERO_DOCUMENTO,
							  ...
							  End EMAIL,
							  case  
								when SUBSTRING(CNPJ_EMITENTE, 1,8) = '05103939' then 'Zamboni'
								when SUBSTRING(CNPJ_EMITENTE, 1,8) = '05041773' then 'FAP'
								when SUBSTRING(CNPJ_EMITENTE, 1,8) = '14741172' then 'Mix'
							end as Filial
...
$pinta = mssql_query("select top 22 row_number() over (order by DATA_EMISSAO)  as linha from            sgc..nfe_novo (nolock) where CNPJ_CPF_DESTINATARIO = $cgc");
...
echo "<font face='verdana' size='1'><table width='100%'  border='0' cellspacing='1' cellpadding='1'>
       <tr>
        <td bgcolor=$bgcolor_topo height='20'><center><font face='verdana' size='2'>E-mail</center></font></td>
		 <td bgcolor=$bgcolor_topo height='20'><center><font face='verdana' size='2'>Emitente</center></font></td>
...
echo "<tr>
		...
		<td bgcolor=$bgcolor><center><font face='verdana' size='2'>" .$dados[7]. "</center></font></td>
