...
$email = $_POST["email"];
$emailok = strtolower(rtrim(ltrim($email)));
...
/// testa campo email em branco
	if ($senhamais == "" or $email == "" ) {
		echo "<script>alert('Favor verificar os campos E-mail e Senha!'); window.history.go(-1); </script>n";
		exit;
		}		
	else {
		...
		// se existir updata
					 mssql_query("INSERT INTO sgc..cliente_xml 
			  				VALUES ('$cgc', '$emailok', 'A', Getdate(),'','','','','N','$senhamais')");
