...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    if old not in text:
        raise SystemExit(f'Patch abortado: trecho esperado não encontrado em {label}.')
    return text.replace(old, new, 1)

for env_file in ['.env.example', '.env.hml.example']:
    text = read(env_file)
    text, count = re.subn(
    ...
    if count != 1:
        raise SystemExit(f'Patch abortado: LEGACY_BOLETO_BASE_URL não encontrado em {env_file}.')
    write(env_file, text)

config_path = 'apps/boleto_api/src/config.ts'
...
db_path = 'apps/boleto_api/src/database.ts'
db = read(db_path)
pattern = re.compile(
...
export async function customerPortal'''
db, count = pattern.subn(replacement, db, count=1)
if count != 1:
    raise SystemExit('Patch abortado: função legacyBoletoAccess não encontrada em database.ts.')
write(db_path, db)

server_path = 'apps/boleto_api/src/server.ts'
...
asset_pattern = re.compile(
    r"\n\s{4}if \(request\.method === 'GET' && url\.pathname === '/api/boleto-asset'\) \{[\s\S]*?\n\s{4}\}\n\n(?=\s{4}if \(request\.method === 'GET' && url\.pathname === '/api/boleto'\))",
    re.MULTILINE
)
server, count = asset_pattern.subn('\n', server, count=1)
if count != 1:
    raise SystemExit('Patch abortado: rota /api/boleto-asset não encontrada em server.ts.')

old_route = '''      const access = await legacyBoletoAccess(session.cnpj, nossoNumero, banco, empresa);
      if (!access) {
...
new_route = '''      const record = await boletoRecord(session.cnpj, nossoNumero, banco, empresa);
      if (!record) {
        json(response, request, 404, { erro: 'Boleto não encontrado para este cliente.' });
        return;
      }
      ...
      await logAccess('log_boleto', session.cnpj, `Documento visualizado: ${nossoNumero}; Banco: ${banco}; Empresa: ${empresa}`);
      html(response, 200, document);'''
server = replace_once(server, old_route, new_route, server_path)
server = replace_once(
    server,
...
old_readme = '''A consulta lista os títulos disponíveis no SQL Server. A emissão de um arquivo
de cobrança não está implementada nesta versão e dependerá de uma fonte nativa
ou URL disponibilizada pela base.'''
new_readme = '''A consulta lista os títulos disponíveis no SQL Server. A visualização e a
...
readme = replace_once(readme, old_readme, new_readme, readme_path)
